...
export default function FormularioOficio({ onCerrar, onGuardado }: Props) {
  ...
  useEffect(() => { setDispositivoIdx(0); }, [legajoId]);
  ...
  useEffect(() => {
    if (tipoConsulta === "linea") setNumeroLinea(lineaActual);
  ...
  async function guardar() {
    ...
    if (tipoConsulta === "imei" && !imeiActual) { setError("El dispositivo no tiene IMEI cargado"); return; }
    ...
    try {
      const res = await fetch("/api/oficios", {
        ...
        body: JSON.stringify({
          ...
          imeiSeleccionado: tipoConsulta === "imei"  ? imeiActual  : "",
          columnas: "[]",
        }),
      });
