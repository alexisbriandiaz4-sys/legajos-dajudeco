...
function FormularioEstafa({ onCerrar, onGuardado, usuarios }: { onCerrar: () => void; onGuardado: () => void; usuarios: UsuarioSimple[] }) {
  ...
  return (
    <ModalFormulario titulo="Nuevo Registro — Estafa" onCerrar={onCerrar} onGuardar={guardar} guardando={guardando}>
      ...
      <div className="col-span-2 space-y-1">
        <label className="text-[var(--text-muted)] text-xs uppercase tracking-wide">Otros CBU</label>
        <textarea value={form.otrosCbu ?? ""} onChange={e => set("otrosCbu", e.target.value)} rows={2} className="w-full px-3 py-2 bg-[var(--bg-tertiary)] border border-[var(--border)] rounded-lg text-[var(--text-primary)] text-sm focus:outline-none focus:border-blue-500 resize-none" />
      </div>
      <SelectorUsuario usuarios={usuarios} value={form.asignadoA ?? ""} onChange={v => set("asignadoA", v)} />
    </ModalFormulario>
  );
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border)] w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-3 p-6 border-t border-[var(--border)]">
          <button onClick={onCerrar} className="flex-1 px-4 py-2 bg-[var(--bg-tertiary)] text-[var(--text-primary)] rounded-lg text-sm hover:bg-[var(--bg-tertiary)] transition-colors">Cancelar</button>
