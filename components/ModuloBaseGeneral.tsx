        ...
        {([
          ...
          <button key={id} onClick={() => setPestana(id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              pestana === id ? "bg-blue-600 text-white" : "text-gray-400 hover:text-white hover:bg-gray-800"
            }`}>
...
function TabTelefonia({ esAdmin }: { esAdmin: boolean }) {
          ...
          <>
            ...
            {esAdmin && (
              <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">
                {r.asignadoA ? (usuarios.find(u => u.id === r.asignadoA)?.nombre ?? "—") : <span className="text-gray-600">Sin asignar</span>}
              </td>
            )}
          </>
        )}
...
function TabEstafas({ esAdmin }: { esAdmin: boolean }) {
  const [datos, setDatos] = useState(DATOS_INICIALES);
  const [page, setPage] = useState(1);
  const [busqueda, setBusqueda] = useState("");
...
function TabGeneral({ esAdmin }: { esAdmin: boolean }) {
              ...
              {cargando ? (
                ...
                <tr key={`${r.tipo}-${r.id}`}
                  ...
                  <td className="px-4 py-3 font-medium">
                    <span className={!r.visto && r.asignadoA ? "text-white" : "text-white"}>{r.victima ?? "—"}</span>
                    {!r.visto && r.asignadoA && <span className="ml-2 px-1.5 py-0.5 rounded text-xs bg-blue-500 text-white">Nuevo</span>}
                  </td>
...
function TablaConPaginacion({ cargando, datos, page, setPage, columnas, renderFila, onVer, onEliminar, mensajeVacio, esAdmin, usuarios }: {
              ...
              <tr key={r.id}
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2 justify-end items-center">
                    {!r.visto && r.asignadoA && (
                      <span className="px-1.5 py-0.5 rounded text-xs bg-blue-500 text-white">Nuevo</span>
                    )}
                    <button onClick={() => onVer(r)} className="p-1.5 text-gray-400 hover:text-blue-400 transition-colors"><Eye className="w-4 h-4" /></button>
                    {onEliminar && <button onClick={() => onEliminar(r)} className="p-1.5 text-gray-400 hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>}
                  </div>
                </td>
              </tr>
            ))}
...
function ModalDetalleTelefonia({ registro: r, onCerrar, usuarios }: { registro: RegistroTelefonia; onCerrar: () => void; usuarios: UsuarioSimple[] }) {
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-700 sticky top-0 bg-gray-800">
          <div>
            ...
            <p className="text-gray-400 text-sm">{r.victima ?? "Sin víctima"} · Asignado a: {nombreAsignado}</p>
          </div>
...
function ModalDetalleEstafa({ registro: r, onCerrar, usuarios }: { registro: RegistroEstafa; onCerrar: () => void; usuarios: UsuarioSimple[] }) {
  const nombreAsignado = r.asignadoA ? (usuarios.find(u => u.id === r.asignadoA)?.nombre ?? "—") : "Sin asignar";
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
...
function SelectorUsuario({ usuarios, value, onChange }: { usuarios: UsuarioSimple[]; value: string; onChange: (v: string) => void }) {
  return (
    <div className="col-span-2 space-y-1">
      <label className="text-gray-400 text-xs uppercase tracking-wide">Asignar a investigador</label>
...
function FormularioTelefonia({ onCerrar, onGuardado, usuarios }: { onCerrar: () => void; onGuardado: () => void; usuarios: UsuarioSimple[] }) {
...
function FormularioEstafa({ onCerrar, onGuardado, usuarios }: { onCerrar: () => void; onGuardado: () => void; usuarios: UsuarioSimple[] }) {
  const [form, setForm] = useState<Record<string, string>>({});
  const [guardando, setGuardando] = useState(false);
  const set = (k: string, v: string) => setForm(f => ({ ...f, [k]: v }));
  const guardar = async () => {
