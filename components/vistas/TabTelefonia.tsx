  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <input type="text" placeholder="Filtrar por causa/hecho..." value={filtroCausa} onChange={e => setFiltroCausa(e.target.value)}
...
function FormularioTelefonia({ onCerrar, onGuardado, esAdmin }: {
  onCerrar: () => void;
  onGuardado: () => void;
  esAdmin: boolean;
}) {
  ...
  const [investigadores, setInvestigadores] = useState<{ id: string; nombre: string }[]>([]);

  useEffect(() => {
    if (esAdmin) {
      fetch('/api/usuarios')
        .then(r => r.json())
        .then(data => setInvestigadores(data.filter((u: any) => u.activo)))
        .catch(() => {});
    }
  }, [esAdmin]);
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-xl border border-gray-700 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 grid grid-cols-2 gap-4">
          ...
          {/* Selector de investigador — solo admins */}
          {esAdmin && (
            <div className="col-span-2 space-y-1">
          ...
          <div className="col-span-2 space-y-1">
            ...
            <textarea value={form.procedimientos ?? ""} onChange={e => set("procedimientos", e.target.value)} rows={3}
              className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:border-blue-500 resize-none" />
          </div>
        </div>
